...
public class DegribBundle extends DataBundle<File> {
	...
	public DegribBundle(File file, DegribVariable variable, Date date, int message){
		super(file,date);
	...
	public DegribBundle(DegribBundle db){
		this(db.getData().getAbsoluteFile(), new DegribVariable(db.getVariable()), db.getDate(), db.getMessage());
